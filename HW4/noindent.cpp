...
int main (int argc, char *argv[]) {
	...
	for (int z = 0; z < Generation; z++) {
		...
		for (int b = 2; b < POP_SIZE; b++) {
			...
			// Get Child
			// Just an Adaptor for using Dr. Heckendorn's pmx function
			for (int q = 0; q < 26; q++) {
				arr1[q] = (int)(Parent1[q] - 65);
				arr2[q] = (int)(Parent2[q] - 65);
			}
    		// Permutation Based Uniformed Crossover
    		pmx(arr1, arr2, int_out, XOVER_RATE, 26);

    		// Adaptoer, convert it back
    		for (int q = 0; q < 26; q++) {
				out[q] = int_out[q] + 65;
			}
			out[26] = '\0';
